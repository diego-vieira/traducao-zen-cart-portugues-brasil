 ...
 */
  ...
  define('SUCCESS_PRODUCT_UPDATE_PRODUCTS_PRICE_SORTER', '<strong>Successful</strong> update for Produtos Price Sorter Values');
  define('SUCCESS_PRODUCT_UPDATE_PRODUCTS_VIEWED', '<strong>Successful</strong> reset of Produtos Viewed to 0');
  define('SUCCESS_PRODUCT_UPDATE_PRODUCTS_ORDERED', '<strong>Successful</strong> reset of Produtos Ordered to 0');
  define('SUCCESS_UPDATE_ALL_MASTER_CATEGORIES_ID', '<strong>Successful</strong> reset of all Master Categorias for Linked Produtos');
  define('SUCCESS_UPDATE_COUNTER', '<strong>Successful</strong> Counter Updated to: ');
  define('SUCCESS_CLEAN_ADMIN_ACTIVITY_LOG', '<strong>Successful</strong> Atualizar of the Admin Activity log');

  define('ERROR_CONFIGURATION_KEY_NOT_FOUND', '<strong>Erro:</strong> No matching Configuration Keys were found ...');
  define('ERROR_CONFIGURATION_KEY_NOT_ENTERED', '<strong>Erro:</strong> No Configuration Key or Text was entered to search for ... Search was terminated');

  define('TEXT_INFO_COUNTER_UPDATE', '<strong>Atualizar Counter</strong><br />to to a new value: ');
  define('TEXT_INFO_PRODUCTS_PRICE_SORTER_UPDATE', '<strong>Atualizar ALL Produtos Price Sorter</strong><br />to be able to sort by displayed prices: ');
  define('TEXT_INFO_PRODUCTS_VIEWED_UPDATE', '<strong>Reset ALL Produtos Viewed</strong><br />Reset Produto Viewed Counts to 0: ');
  define('TEXT_INFO_PRODUCTS_ORDERED_UPDATE', '<strong>Reset ALL Produtos Ordered</strong><br />Reset Produto Ordered Counts to 0: ');
  define('TEXT_INFO_MASTER_CATEGORIES_ID_UPDATE', '<strong>Reset ALL Produtos Master Categorias ID</strong><br />to be used for Linked Produtos and Pricing: ');
  define('TEXT_INFO_ADMIN_ACTIVITY_LOG', '<strong>Empty Admin Activity Log table from the database<br />ATENÇÃO: Be sure to backup your database before running this update!</strong><br />The Admin Activity Log is a tracking method that records activity in the Admin. Due to its nature it can become very large, very fast and does need to be cleaned out from time to time.<br />Warnings are given at 50,000 records or 60 days, which ever happens first.');

  define('TEXT_ORDERS_ID_UPDATE', '<strong>Reset Current Order ID</strong>');
  define('TEXT_INFO_ORDERS_ID_UPDATE', '<strong>NOTA: Before updating the Current Order ID ...</strong><br /><br />Create a test order first. Then, using this Test Order ID complete the information below.<br />The Novo Order ID for the next Real Order should be entered as 1 less than the Order ID you wish to use.<br /><strong>Example:</strong> If you want your next Real Order ID to be 1225, enter the number 1224<br /><br /><strong>ATENÇÃO:</strong> You can only reset the Order ID forward, and not backwards.<br />So if you set the Orders ID to 25, then change it again to 20, the next Orders ID will still be 26.');
  define('TEXT_OLD_ORDERS_ID', 'Old Order ID');
  define('TEXT_NEW_ORDERS_ID', 'Novo Order ID');
  ...
  define('TEXT_INFO_CONFIGURATION_UPDATE', '<strong>NOTA:</strong> CONSTANTS are written in uppercase.<br />Language file lookups may be an alternative search when nothing has been found in the database tables.');
  ...
  define('TEXT_INFO_CONFIGURATION_UPDATE_FILES', '<strong>NOTA:</strong> Language file lookups maybe upper or lower case');

  define('TABLE_TITLE_KEY', '<strong>Key:</strong>');
  define('TABLE_TITLE_TITLE', '<strong>Title:</strong>');
  define('TABLE_TITLE_DESCRIPTION', '<strong>Descrição:</strong>');
